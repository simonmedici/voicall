...
async function createProducts() {
  ...
  const products = [
    {
      ...
      description: 'Für kleine Praxen - 500 Minuten/Monat',
      metadata: { tier: 'starter', minutes: '500' },
      price: 19900,
    },
    {
      name: 'Voicall Pro',
      description: 'Für größere Praxen - 1500 Minuten/Monat',
      metadata: { tier: 'pro', minutes: '1500' },
      price: 34900,
    },
    {
  ...
  for (const productData of products) {
    ...
    if (existingPrices.data.length > 0) {
      ...
      console.log(`Price already exists for "${productData.name}": ${price.id}`);
    } else {
